...
#[acmd_func(
    ...
    battle_object_kind = FIGHTER_KIND_EFLAME, 
    animation = "catch",
    animcmd = "game_catch")]
pub fn eflame_catch(fighter: &mut L2CFighterCommon) {
    acmd!
    ({
        frame(Frame=5)
        if(is_excute)
...
pub fn eflame_catchdash(fighter: &mut L2CFighterCommon) {
    acmd!
    ({
        frame(Frame=11)
        if(is_excute){
...
pub fn eflame_catchturn(fighter: &mut L2CFighterCommon) {
    acmd!
    ({
        frame(Frame=12)
        if(is_excute){
